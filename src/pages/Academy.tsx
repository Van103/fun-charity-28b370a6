...
export default function Academy() {
  ...
  const { posts, isLoading: postsLoading, isFetchingNextPage, hasNextPage, fetchNextPage } = useInfiniteFeedPosts({});
  ...
  const loadMoreRef = useIntersectionObserver(loadMore, { rootMargin: "200px" });
  ...
  return (
    <>
      <Helmet>
        <title>Fun Academy - FUN Charity</title>
        <meta name="description" content="Học hỏi và chia sẻ kiến thức với cộng đồng FUN Charity" />
      </Helmet>
      <div className="min-h-screen bg-background">
        <Navbar />
        <main className="pt-20 pb-12">
          <div className="container mx-auto px-4">
            <div className="flex gap-6">
              <div className="hidden lg:block"><LeftSidebar profile={profile} /></div>
              <div className="flex-1 max-w-2xl mx-auto lg:mx-0">
                <PullToRefresh onRefresh={handleRefresh}>
                  <div className="space-y-6">
                    ...
                    <div className="space-y-6">
                      {postsLoading ? <PostCardSkeletonList count={3} /> : posts && posts.length > 0 ? (
                        <>
                          {posts.map((post) => <SocialPostCard key={post.id} post={post} />)}
                          <div ref={loadMoreRef} className="py-4">
                            {isFetchingNextPage && <PostCardSkeleton />}
                            {!hasNextPage && posts.length > 0 && <p className="text-center text-sm text-muted-foreground">Bạn đã xem hết tất cả bài viết 🎉</p>}
                          </div>
                        </>
                      ) : (
              ...
              <div className="hidden xl:block"><RightSidebar /></div>
            </div>
          </div>
        </main>
        <Footer />
